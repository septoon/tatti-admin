  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        {/* <button onClick={onSave} disabled={saving} className="ml-auto px-3 py-2 border rounded bg-black text-white disabled:opacity-50">
          {saving ? 'Сохранение...' : 'Сохранить'}
        </button> */}
      </div>

      {/* Desktop table */}
      <div className="overflow-auto border rounded hidden md:block">
          ...
          <tbody>
            {items.map(it => (
              <tr key={it.id} className="border-t">
                ...
                <td className="p-2">
                  <input className="border rounded px-2 py-1 w-full" value={it.title} onChange={e=>updateItem(it.id,{ title: e.target.value })} />
                </td>
                <td className="p-2">
                  <input type="number" className="border rounded px-2 py-1 w-24" value={it.price} onChange={e=>updateItem(it.id,{ price: Number(e.target.value) })} />
                </td>
                <td className="p-2">
                  <textarea
